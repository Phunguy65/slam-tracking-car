...
# Lazy imports for ML libraries (may not be available in test environments)
try:
...
class EnrollmentNode(Node):
    ...
    # Status constants matching EnrollmentStatus.msg
    STATUS_IDLE = 0
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "scan_frames", 10
        )  # Number of frames to average for embedding
        ...
        self.no_face_timeout = 0.5  # seconds
        ...
        # Timer for status publishing and timeout checks
    ...
    def _init_database(self, db_path: str):
        ...
        # Create tracking target table (single row)
        cursor.execute("""
        ...
        # Insert default tracking target row if not exists
        cursor.execute("""
    ...
    def _image_callback(self, msg: CompressedImage):
        ...
        # Detect face using YOLOv8 (class 0 is person, but we use InsightFace for face detection)
        # Actually, use InsightFace directly for face detection + embedding
        faces = self.face_app.get(frame)

        if len(faces) == 0:
            # No face detected
            if self.current_status in [self.STATUS_FACE_DETECTED, self.STATUS_SCANNING]:
                # Face was being tracked, now lost
                self.current_status = self.STATUS_NO_FACE
                self._reset_scan()
            elif self.current_status == self.STATUS_READY:
                # Keep ready state even if face temporarily not visible
                pass
            else:
                self.current_status = self.STATUS_IDLE
            return
        ...
        if self.current_status == self.STATUS_SCANNING:
            ...
            if face.embedding is not None:
                ...
                if len(self.scan_embeddings) >= scan_frames:
                    ...
                    # Create thumbnail (crop face area, resize to 128x128)
    ...
    def _add_person_callback(self, request, response):
        ...
        # Generate UUID and store in database
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO persons (id, name, embedding, thumbnail, created_at) VALUES (?, ?, ?, ?, ?)",
                (
                    person_id,
                    request.name.strip(),
    ...
    def _remove_person_callback(self, request, response):
        ...
        try:
            ...
            # Clear tracking target if this person is the target
            cursor.execute(
    ...
    def _list_persons_callback(self, request, response):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, name, thumbnail, created_at FROM persons ORDER BY created_at DESC"
            )
    ...
    def _set_target_callback(self, request, response):
        ...
        try:
            ...
            # If person_id is empty, clear the target
            if not request.person_id:
                cursor.execute(
